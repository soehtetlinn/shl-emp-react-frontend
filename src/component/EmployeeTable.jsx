...
class AdminEmployeeTable extends Component {
  ...
  loadEmployeeData = () => {
    axios
      .get("https://i5z6ubo553.execute-api.us-east-2.amazonaws.com/prod/api/employee")
      .then(response => {
        this.employeeObj = response.data.body.message;
        console.log("response", response.data.body.message);
        this.setState({ employeeData: response.data.body.message });
        this.setState({ loading: false });
        this.rowDataT = [];
        this.employeeObj.map(item => {
          let temp = {
            item,
            ...
            TerminateDateData: item["TerminateDateData"] ? item["TerminateDateData"]["S"] : "Not Available ",
          };
